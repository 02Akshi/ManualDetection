...
# sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues', xticklabels=['Manual Absent', 'Manual Present'], yticklabels=['Manual Absent', 'Manual Present'])
...
# Get the list of all test image filenames
test_image_filenames = test_data.filenames

false_positives = [] # Predicted Manual Present, Actual Manual Absent
false_negatives = [] # Predicted Manual Absent, Actual Manual Present
...
for i in range(len(test_labels)):
    ...
    # False Positive (FP): True is Manual Absent (0), Predicted is Manual Present (1)
    if true_label == class_indices['Manual Absent'] and predicted_label == class_indices['Manual Present']:
        false_positives.append(filename)
    # False Negative (FN): True is Manual Present (1), Predicted is Manual Absent (0)
    elif true_label == class_indices['Manual Present'] and predicted_label == class_indices['Manual Absent']:
        false_negatives.append(filename)

print("\n--- False Positives (Predicted: Manual Present, Actual: Manual Absent) ---")
if false_positives:
    for fp_name in false_positives:
        print(f"- {fp_name}")
else:
    print("No False Positives found.")

print("\n--- False Negatives (Predicted: Manual Absent, Actual: Manual Present) ---")
if false_negatives:
    for fn_name in false_negatives:
        print(f"- {fn_name}")
else:
    print("No False Negatives found. (This is excellent!)")

# Save misclassified image names to a file
misclassified_report_path = os.path.join(output_dir, "misclassified_images.txt")
with open(misclassified_report_path, "w") as f:
    f.write("--- False Positives (Predicted: Manual Present, Actual: Manual Absent) ---\n")
    if false_positives:
        for fp_name in false_positives:
            f.write(f"- {fp_name}\n")
    else:
    ...
    if false_negatives:
        ...
        f.write("No False Negatives found. (This is excellent!)\n")
print(f"Misclassified image names saved to: {misclassified_report_path}")
